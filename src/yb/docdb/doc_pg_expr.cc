...
// Deserialized Postgres expression paired with type iformation to convert results to DocDB format
 ...
 public:
  ...
  CHECKED_STATUS PreparePgRowData(const QLTableRow& table_row) {
    ...
    if (row_ctx_ == nullptr) {
      ...
      VLOG(1) << "Created row memory context";
    ...
    // Exit if we have no column references
    if (var_map_.empty()) {
      return Status::OK();
    }
